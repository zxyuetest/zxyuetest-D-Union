...
import { Wallet, CalendarDays, MessageSquarePlus, HeartHandshake, Award, Home as HomeIcon, Compass, User, Bell, ChevronRight, CreditCard, Receipt, Smile, Scale, Megaphone, FileCheck, LogOut, X, ChevronLeft } from 'lucide-react';
...
export default function Home({ onNavigate, user, onLogout }: { onNavigate: (view: string) => void, user: UserType, onLogout: () => void }) {
  ...
  const handleMenuClick = (id: string) => {
    if (['dues', 'activities', 'suggestions', 'points', 'matchmaking', 'insurance', 'counseling', 'legal_aid', 'fund_approval'].includes(id)) {
      onNavigate(id);
    } else {
      alert('该功能正在开发中，敬请期待！');
    }
